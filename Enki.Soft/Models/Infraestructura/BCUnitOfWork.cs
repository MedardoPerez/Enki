...
namespace ENKI.SOFT.Models.Infraestructura
{
    public class BCUnitOfWork : DbContext
    {
        ...
        private string GetTransactionTableName(string tname)
        {
            ...
            tname = tname.Replace("`", string.Empty);

            string result = $"{tname}_Transacciones";
            return result;
        }
        ...
        private SqlCommandInfo GetAuditHeaderCommandInfo(Transaction transaction)
        {
            const string sqlInsert =
                "insert into  LogTransacciones(TransaccionUId, TipoTransaccion, FechaTransaccion," +
        ...
        {
            const string sqlInsert =
                "insert into  LogTransaccionesDetalle(TransaccionUId,TipoTransaccion, EntidadDominio, DescripcionTransaccion) " +
                                       "values({0}, {1}, {2},{3})";
